...
module Sayable
  ...
  module ClassMethods
    ...
    def say_random_tip(selector, nick, keywords = '', options =())
      ...
      replies = if !!tip.parent
        "In reply to: #{tip.parent.body.strip}\n"
      else
        ''
      end
      ...
      if tip_body =~ /^server/i
        ...
        play_sound(selector, 'minecraft:entity.generic.explode')
    ...
    def tips(selector = '@a', keywords = '')
      tips = if keywords.empty?
        ...
        Message::Tip.query(keywords = keywords.strip).except(:select)
      end

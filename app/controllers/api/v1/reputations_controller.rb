class Api::V1::ReputationsController < Api::V1::ApiController
  def index
    ...
    @reputations = @reputations.where(truster_id: Player.any_nick(any_truster_nick)) if any_truster_nick.present?
    @reputations = @reputations.where(trustee_id: Player.any_nick(any_trustee_nick)) if any_trustee_nick.present?
  end

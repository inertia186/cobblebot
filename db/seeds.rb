...
ServerCallback::AnyPlayerEntry.send method, name: 'Calc', pattern: "/^=(.+)/", command: "mathstring = \"%1%\".strip\ncalculator = Dentaku::Calculator.new\nanswer = calculator.evaluate(mathstring)\nanswer = answer.to_i if answer == answer.to_i\nsay \"@a\", \"\#{answer}=\#{mathstring}\"", system: 't'

# AI
ServerCallback::PlayerCommand.send method, name: 'Predict Rate', pattern: "/^@server predict rate ([a-zA-Z0-9_]+) ([a-zA-Z0-9_]+)$/i", command: "truster_nick = '%1%'\ntrustee_nick = '%2%'\ntruster = Player.search_any_nick(truster_nick).first\ntrustee = Player.search_any_nick(trustee_nick).first\n\nif truster.nil?\n  say '@a', \"Unknown player: \#{truster_nick}\"\n  return\nend\n\nif trustee.nil?\n  say '@a', \"Unknown player: \#{trustee_nick}\"\n  return\nend\n\nprediction = truster.predict_reputation trustee.nick\n\nif prediction.nil?\n  say '@a', \"Cannot predict what \#{truster.nick} would rate \#{trustee.nick}.\"\nelse\n  say '@a', \"Predicting \#{truster.nick} to rate \#{trustee.nick} \#{prediction}\"\nend", help_doc_key: 'predict', help_doc: "Usage: @server predict rate <player> <other player>\nUsage: @server predict death <player>\n\nPredictions use an automatic classifier and past data to make educated guesses about the future.\n\nCobbleBot implements the ID3 algorithm (Quinlan) as one of its automatic classifiers.  Given a set of preclassified examples, it builds a top-down induction of decision tree, biased by the information gain and entropy measure.", system: 't'
ServerCallback::PlayerCommand.send method, name: 'Predict Death', pattern: "/^@server predict death ([a-zA-Z0-9_]+)$/i", command: "nick = '%1%'\nplayer = Player.search_any_nick(nick).first\n\nif player.nil?\n  say '@a', \"Unknown player: \#{nick}\"\n  return\nend\n\nprediction = player.predict_death\n\nif prediction.nil?\n  say '@a', \"Cannot predict when \#{player.nick} will die.\"\nelse\n  if Server.player_nicks.include?(player.nick)\n    say '@a', \"Predicting \#{player.nick} to die in \#{prediction}.\"\n  else\n    say '@a', \"Predicting \#{player.nick} to die \#{prediction} after logging in.\"\n  end\nend", system: 't'
